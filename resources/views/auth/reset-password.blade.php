@extends('layouts.guest')

@section('title', 'Nexo Estoque — Nova senha')

@section('content')
<div class="auth-card">
  <div class="auth-form">
    <div class="auth-brand">
      <div class="auth-logo"><img src="{{ asset('images/logo.png') }}" alt="Nexo Estoque"></div>
      <span class="auth-brand__name">Nexo Estoque</span>
    </div>
    <h1 class="auth-title">Nova senha</h1>
    <p class="auth-sub">Defina a nova senha da sua conta.</p>

    <form method="POST" action="{{ route('password.update') }}">
      @csrf
      <input type="hidden" name="token" value="{{ $token }}">

      @if($errors->any())
        <div class="auth-error"><span>⚠️</span><span>{{ $errors->first() }}</span></div>
      @endif

      <div class="auth-field">
        <label>Email</label>
        <div class="auth-input-wrap">
          <span class="ic">✉️</span>
          <input class="auth-input @error('email') error @enderror"
                 name="email" type="email" placeholder="seu@email.com"
                 value="{{ old('email', $email) }}" autocomplete="email" required>
        </div>
      </div>

      <div class="auth-field">
        <label>Nova senha</label>
        <div class="auth-input-wrap">
          <span class="ic">🔒</span>
          <input class="auth-input @error('password') error @enderror"
                 id="rpPass" name="password" type="password" placeholder="••••••••"
                 autocomplete="new-password" required autofocus>
          <button class="auth-eye" type="button" tabindex="-1" onclick="togglePass('rpPass', this)">👁</button>
        </div>
      </div>

      <div class="auth-field">
        <label>Confirmar nova senha</label>
        <div class="auth-input-wrap">
          <span class="ic">🔒</span>
          <input class="auth-input"
                 id="rpPass2" name="password_confirmation" type="password" placeholder="••••••••"
                 autocomplete="new-password" required>
          <button class="auth-eye" type="button" tabindex="-1" onclick="togglePass('rpPass2', this)">👁</button>
        </div>
      </div>

      <button class="auth-btn" type="submit" style="margin-top:8px;">Redefinir senha →</button>

      <div class="auth-foot"><a href="{{ route('login') }}">Voltar ao login</a></div>
    </form>
  </div>

  <div class="auth-aside">
    <div class="auth-aside__head">
      <h2>Controle total do seu <span>estoque de peças</span></h2>
      <p>Do recebimento à saída, tudo em um só lugar.</p>
    </div>
    <div class="auth-aside__img"><img src="{{ asset('images/login-illustration.png') }}" alt="Gestão de estoque"></div>
  </div>
</div>

<script>
function togglePass(id, btn) {
  const i = document.getElementById(id);
  const p = i.type === 'password';
  i.type = p ? 'text' : 'password';
  btn.textContent = p ? '🙈' : '👁';
}
</script>
@endsection
